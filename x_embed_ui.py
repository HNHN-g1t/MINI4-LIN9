...
(() => {
  ...
  function measureWall(){
    ...
    const col = Math.max(1, (WALL.clientWidth - WALL_GAP * (WALL_COLS - 1)) / WALL_COLS);
  ...
  // 押した1件を先頭に、残りをシャッフルして並べる。
  // DOMは動かさず order だけ振る（動かすとiframeが読み込み直しになる）。
  function orderWall(leadId){
    if(!wallCells.length) return;
    const rest = wallCells.filter(c => c.id !== leadId);
    for(let i = rest.length - 1; i > 0; i--){
      const j = Math.floor(Math.random() * (i + 1));
      const t = rest[i]; rest[i] = rest[j]; rest[j] = t;
    }
    const lead = wallCells.find(c => c.id === leadId);
    (lead ? [lead].concat(rest) : rest).forEach((c, i) => { c.cell.style.order = i; });
  }
