...
#xFeatured{--xw:250px;--xs:1;--xgap:6px;--xh:280px;
...
(() => {
  ...
  function measureWall(){
    ...
    if(!wallDone) wallNatural = Math.round(Math.min(550, Math.max(NATURAL, col)));
  ...
  // 先頭の1段は、TOPの帯に出ていた並びをそのまま持ってくる。
  // 押した1件があれば、その中でいちばん前に回す。
  // 2段目以降はシャッフル。DOMは動かさず order だけ振る
  // （動かすとiframeが読み込み直しになる）。
  function orderWall(leadId){
    if(!wallCells.length) return;
    const byId = id => wallCells.find(c => c.id === id);

    // 帯に出ていたぶんを、帯と同じ順で先頭に置く
    let head = (bandIds || []).map(byId).filter(Boolean);
    if(leadId){
      const i = head.findIndex(c => c.id === leadId);
    ...
    const rest = wallCells.filter(c => !headIds.has(c.id));
    for(let i = rest.length - 1; i > 0; i--){
      const j = Math.floor(Math.random() * (i + 1));
      const t = rest[i]; rest[i] = rest[j]; rest[j] = t;
    }
    head.concat(rest).forEach((c, i) => { c.cell.style.order = i; });
  }
  ...
  async function buildWall(){
    ...
    // 1件でも応答が返らないと、この先の並び替えまで進めなくなる。
    // 帯と同じように時間で打ち切る（描画済みのものはそのまま使える）。
    let wt = null;
